#ifndef H_SIMPLETIMER
#define H_SIMPLETIMER
...
class SimpleTimer {
 public:
  ...
  // Timer will call function 'f' with parameter 'p' every 'd' milliseconds
  // forever returns the timer number (numTimer) on success or -1 on failure (f
  // == NULL) or no free timers
  ...
  // Timer will call function 'f' with parameter 'p' after 'd' milliseconds one
  // time returns the timer number (numTimer) on success or -1 on failure (f ==
  // NULL) or no free timers
  ...
  // Timer will call function 'f' with parameter 'p' every 'd' milliseconds 'n'
  // times returns the timer number (numTimer) on success or -1 on failure (f ==
  // NULL) or no free timers
 ...
 private:
  ...
  typedef struct {
    unsigned long prev_millis;  // value returned by the millis() function in
    ...
    unsigned toBeCalled;  // deferred function call (sort of) - N.B.: only used
                          // in run()
  } timer_t;
